...
local/bin/emerge: build/portage/src/configure .bootstrap_python.stamp .bootstrap_wget.stamp .bootstrap_findutils.stamp .bootstrap_coreutils.stamp .bootstrap_sed.stamp \
	...
	if COLLISION_IGNORE='**' ${SAGE_ROOT}/sage -emerge --oneshot legacy-spkg/portage; then \
	   PATH=${SAGE_LOCAL}/bin:$$PATH sed -i 's/_enable_ipc_daemon = True/_enable_ipc_daemon = False/g' ${SAGE_LOCAL}/lib/portage/pym/_emerge/AbstractEbuildProcess.py ; \
           true; \
        else \
           rm local/bin/emerge; \
...
.rebuilt_gccs_dependencies.stamp:
	if gcc --version | grep 4.6 > /dev/null; then \
            ...
            if gcc --version | grep 4.7 > /dev/null; then \
                ...
                ${SAGE_ROOT}/sage -emerge --noreplace --oneshot --noreplace legacy-spkg/gcc; \
                ${SAGE_ROOT}/sage -emerge --oneshot legacy-spkg/mpir legacy-spkg/mpfr legacy-spkg/mpc legacy-spkg/zlib legacy-spkg/gcc; \
            fi; \
        fi
...
sage: local/bin/emerge .rebuilt_gccs_dependencies.stamp
	${SAGE_ROOT}/sage -emerge --noreplace --oneshot --deep --update --keep-going ${SAGE_VERSION_PREFIX}legacy-spkg/sage-full${SAGE_VERSION_SUFFIX}
